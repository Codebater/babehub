...
export default function SurveyModal({ isOpen, onClose }: SurveyModalProps) {
  ...
  const canProceed = () => {
    switch (currentStep) {
      case 1: {
        const revenueOk = formData.isGeneratingRevenue === 'yes' ? formData.monthlyEarnings !== '' : true;
        return (
          formData.isOver18 === 'yes' &&
          formData.isActiveCreator !== '' &&
  ...
  return (
    ...
    >
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-4 text-text-secondary hover:text-primary transition-colors" aria-label={t('survey.close')}>
        ...
        {isSubmitted ? (
          <div className="text-center py-12 animate-fade-in-up flex-grow flex flex-col justify-center">
            ...
            <button onClick={onClose} className="mt-8 bg-primary hover:bg-pink-400 text-white font-bold py-2 px-6 rounded-full transition-all duration-300 self-center">
          ...
          <>
            <div className="text-center mb-4">
              <div className="relative inline-block mb-2">
                ...
                <svg className="absolute -bottom-4 left-0 w-full h-auto text-primary z-0" viewBox="0 0 100 12" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
                  <path d="M2 7 C 35 -3, 65 15, 98 5" stroke="currentColor" strokeWidth="1" strokeLinecap="round" />
                </svg>
              </div>
                ...
                {currentStep === 1 && (
                  <div className={stepContentClass} style={animationStyle}>
                    ...
                    {formData.isGeneratingRevenue === 'yes' && (
                      <div className="animate-fade-in-up" style={animationStyle}>
