...
function Card({ children, delay = '0ms' }: { children: React.ReactNode; delay?: string }) {
...
export default function Hero({ onApplyClick }: HeroProps) {
  ...
  useEffect(() => {
    const id = setInterval(() => setCurrentWordIndex((p) => (p + 1) % words.length), 2500);
        ...
        <div className="relative w-full h-[700px] md:h-[650px] lg:h-[600px] flex items-center justify-center scale-90 sm:scale-100">
            ...
            <Card delay="0.4s">
              <div className="flex justify-between items-center mb-1 sm:mb-2">
                <div className="flex items-center space-x-2">
                  <div className="w-7 h-7 flex items-center justify-center rounded-full bg-blue-400/20">
                    <TwitterIcon className="w-4 h-4 text-blue-400" fill="currentColor" />
                  </div>
              ...
              <svg className="w-full h-10 sm:h-12 lg:h-16" viewBox="0 0 100 40" preserveAspectRatio="none">
                <path d="M0 30 C 20 10, 40 10, 60 25 S 80 30, 100 20" stroke="#F472B6" strokeWidth="2" fill="none" strokeLinecap="round" />
              </svg>
              <div className="flex items-baseline space-x-2 mt-1">
                <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-text-main">58.5M</p>
                <p className="text-xs font-semibold text-green-400">+127.9%</p>
              </div>
            </Card>
          </div>
          ...
          <div className="absolute top-48 -left-4 w-48 transform -rotate-2 lg:top-36 lg:left-4 lg:w-52">
            <Card delay="0.6s">
              ...
              <svg className="w-full h-5 sm:h-6 lg:h-8 mt-1 sm:mt-2" viewBox="0 0 100 20" preserveAspectRatio="none">
                <path d="M0 15 L 10 5 L 20 10 L 30 2 L 40 12 L 50 8 L 60 18 L 70 10 L 80 15 L 90 5 L 100 10" stroke="#4ADE80" strokeWidth="1.5" fill="none" strokeLinecap="round" />
              </svg>
            </Card>
          </div>
          ...
          <div className="relative text-center animate-fade-in-down z-30 max-w-xl">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-text-main mb-6 tracking-tighter leading-tight">
              ...
              <span className="inline-grid h-[1.25em] align-middle overflow-hidden">
                <span className="col-start-1 row-start-1 invisible">{t('hero.word.money')}</span>
                <span
              ...
              <span className="relative inline-block whitespace-nowrap">
                ...
                <svg className="absolute -bottom-2.5 left-0 w-full h-auto text-primary z-0" viewBox="0 0 180 12" fill="none" preserveAspectRatio="none">
                  <path d="M1 8C40 2, 140 2, 179 8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                  <path d="M1 11C40 5, 140 5, 179 11" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                </svg>
              </span>
            </h1>
          ...
          <div className="absolute bottom-12 -left-2 w-52 transform -rotate-3 lg:bottom-12 lg:left-20 lg:w-60">
            <Card delay="0.8s">
              ...
              <div className="flex justify-between items-end mt-1">
                ...
                <button className="bg-secondary p-1.5 sm:p-2 rounded-lg border border-border-color hover:border-primary transition-colors">
                  <TrendingUpIcon className="w-4 h-4 sm:w-5 sm:h-5 text-text-secondary" />
                </button>
              </div>
            </Card>
          </div>
          ...
          <div
            className="absolute bottom-12 -right-2 w-32 h-20 bg-primary rounded-2xl shadow-xl flex flex-col items-center justify-center space-y-1 transform rotate-6 lg:bottom-24 lg:right-16 lg:w-40 lg:h-24 animate-fade-in-up transition-colors duration-700"
            style={{ animationDelay: '1s' }}
          >
            <div className="w-6 h-6 lg:w-8 lg:h-8 bg-white/20 rounded-full flex items-center justify-center">
              <CheckIcon className="w-3 h-3 lg:w-5 lg:h-5 text-white" />
            </div>
