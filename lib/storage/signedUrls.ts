 ...
 * post need to see the images too, but they don't own the file — so we
 * mint a signed URL server-side and embed it in the rendered HTML.
 *
 ...
 *
 * Returns a Map<media_id, signed_url>. Missing entries indicate either an
 * unknown media id or an expired/failed sign — caller should render a
 * graceful fallback (e.g. broken-image placeholder).
 */
export async function getSignedMediaUrls(
  ...
  // De-dupe so a media used in multiple posts only mints once.
  ...
  for (const row of rows) {
    ...
    entry.push({ id: row.id, path: row.storage_path });
  ...
  await Promise.all(
    Array.from(byBucket.entries()).map(async ([bucket, items]) => {
      ...
      signed.forEach((s, idx) => {
        ...
        if (item && s.signedUrl) result.set(item.id, s.signedUrl);
      });
    }),
  );
